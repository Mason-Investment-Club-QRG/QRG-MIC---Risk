...
final_port_returns = final_port_returns.sum(
    axis=1
)  # Creates a data series with the portfolio returns

# Excess Returns
excess = (
    final_port_returns - rf_returns
)  # calculate the excess returns for the time period
excess = excess.to_frame()  # Changes the series to a dataframe

# Sharpe Ratio
sr = (excess.mean() / excess.std()) * math.sqrt(
    trading_days
)  # Sharpe Ratio Formula: [Rp - Rf / std(Rp - Rf) * sqrt(252)]
